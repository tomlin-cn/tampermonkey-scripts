// ==UserScript==
// @name         Shopee印尼注册自动接码助手(定向修复版)
// @namespace    http://tampermonkey.net/
// @version      3.4
// @description  定向修复：修正邮箱提取正则，邮件OTP延迟限次，强化OTP页面监控
// @author       You
...
(function() {
    ...
    function startEmailOtpMonitor() {
        const monitorTimer = setInterval(() => {
            ...
            if (notice && notice.innerText.includes("e-mail")) {
                ...
                state.emailAttemptCount = 0; // 重置计数
                setTimeout(() => {
                    log("🔎 开始执行邮件码接收...");
    ...
    // ---------------------------------------
        ...
        div.innerHTML = `
            ...
            <button id="btn_start" style="width:100%; padding: 8px; background:#ee4d2d; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold; margin-bottom:8px;">开始注册 (取号)</button>
            ...
            onload: function(response) {
                try {
                    ...
                    if (gotPhone) {
                        ...
                        GM_setValue("last_fetched_phone", gotPhone); // 存入数据库，防止跳转后丢失
