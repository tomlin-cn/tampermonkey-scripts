...
// @updateURL    https://raw.githubusercontent.com/tomlin-cn/tampermonkey-scripts/main/tiktok_deactive222222.user.js
// @downloadURL  https://raw.githubusercontent.com/tomlin-cn/tampermonkey-scripts/main/tiktok_deactive222222.user.js
// @version      4.6.8
  ...
  // ====== 0. 自动更新/发布页面逻辑 (全程验证码阻塞版) ======
    ...
    panel.innerHTML = `
        ...
        <div style="display:flex;justify-content:space-between;margin-bottom:8px;align-items:center;">
            ...
            <input type="number" id="tk_view_threshold" value="5" style="width:60px;padding:3px;border-radius:4px;border:none;text-align:center;font-weight:bold;color:blue;">
        </div>
    ...
    setInterval(() => {
        ...
        if (quotaEl) quotaEl.innerText = q > 0 ? `待处理名额: ${q}` : `无需下架 (保留46)`;
    }, 2000);
    ...
    document.getElementById('tk_start_btn').addEventListener('click', async () => {
        ...
        let maxDel = stats.total - 46;
        if (maxDel <= 0) {
            log(`当前活跃总数 ${stats.total} 未超过 46，无需操作`, 'orange');
            return;
        }
